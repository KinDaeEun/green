...
<c:if test="${empty list }">
	<tr class="table-warning"><td colspan="5">게시글이 없습니다</td></tr>
</c:if>
<c:if test="${not empty list}">
	<c:forEach var="board" items="${list }">
		...
		<c:if test="${board.del != 'y' }">
			<!-- 답변글 -->
			<td title="${board.content }"><c:if test="${board.re_level > 0}">
				<img src="${path }/resources/images/level.gif" 
					height="5" width="${board.re_level * 10 }">
				<img src="${path }/resources/images/re.gif">
			</c:if>
				<a href="${path }/view/num/${board.num}/pageNum/${pageNum}"
				 class="btn btn-primary btn-sm">${board.subject}</a>
		...
		<c:forEach var="i" begin="${pb.startPage }" end="${pb.endPage}">
			<c:if test="${pb.currentPage == i }">
				<li class="page-item active">
					<a class="page-link" href="${path}/list/pageNum/${i}?search=${board.search}&keyword=${board.keyword}">${i }</a></li>
			</c:if>
			<c:if test="${pb.currentPage != i }">
				<li class="page-item">
					<a class="page-link" href="${path}/list/pageNum/${i}?search=${board.search}&keyword=${board.keyword}">${i }</a></li>
